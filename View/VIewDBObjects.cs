...
namespace LibraryDB.View
{
    internal class VIewDBObjects
    {
        ...
        public string[] bookLendingLabels = { "ID выдачи (не используется в добавлении)", "Библиотекарь", "Читатель", "Книга", "Кол-во", "Дата выдачи", "Дата возврата" };
       ...
       public void viewModeRowDB(string[] obj, int ID, mode mode)
       {
            switch (choosenTableID)
            {
                ...
                case 8:
                    ...
                    break;
                default:
                    MessageBox.Show($"Возникла ошибка добавления строки:\nНе удалось добавить данные. Возможно, отсутствует подключение к серверу", "Неизвестная ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;
            }
       }
    }
